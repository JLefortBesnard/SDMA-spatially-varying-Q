...
def preprocessing(data_dir, output_dir):
    ...
    result_dir = op.join('results', 'HCP', 'masking')
    if not os.path.exists(op.join(result_dir, 'participant_mask.nii.gz')):
        mask_list = []
        ...
        mask = masking.intersect_masks(mask_list, threshold=1)
        nib.save(mask, op.join(result_dir, 'participant_mask.nii.gz'))

    else:
        mask = nib.load(op.join(result_dir, 'participant_mask.nii.gz'))
    ...
    for idx, img in enumerate(img_list):
        ...
        try:
            ...
            nib.save(res_masked_img, op.join(output_dir, op.basename(img))) # Save original image resampled and normalized
...
data_dir = os.path.join('data', 'HCP', 'group-1_right-hand*tstat.nii')
# overwrite
output_dir = os.path.join('data', 'HCP')
